...
class Wxwidgets(target.AutoBuild):
	...
	configure_flags = target.AutoBuild.configure_flags + ' --enable-unicode --enable-threads --without-subdirs --without-gnomeprint PKG_CONFIG=%(targetdir)s/pkg-config-replacement'
	dependencies = ['libtiff','libpng','gettext']
	patches = ['wxwidgets-2.8.11-spinctlg-1.patch']
